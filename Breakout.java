...
/** Runs the Breakout program. */
	...
	private void checkForCollision(){
		...
		if(collider==paddle){
			if(ball.getY() >= getHeight() - PADDLE_Y_OFFSET - PADDLE_HEIGHT - BALL_RADIUS && ball.getY() < getHeight() - PADDLE_Y_OFFSET - PADDLE_HEIGHT - BALL_RADIUS*2 + 4){	
				vy=-vy;
			}	
	...
	private GObject getCollidingObject(){
		if (gobj!=null&&ball.getY()<getHeight()/2){
			No_bricks--;
			return brick;
		}else if(gobj==paddle/*!=null&&ball.getY()>getHeight()/2*/){
			return paddle;
		}else{
			return null;
		}
